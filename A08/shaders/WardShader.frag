...
*/
    ...
    // Dot products for normal and light/view/half vectors
    float dotLN = max(dot(L, N), 0.0);
    float dotVN = max(dot(V, N), 0.0);
    float dotHN = dot(H, N);
    float dotHTAlphaX = dot(H, T) / alphaT;
    float dotHBAlphaY = dot(H, B) / alphaB;

    // Prevent division by zero at grazing angles
	vec3 Specular;
	if(dotLN == 0.0 || dotVN == 0.0 || dotHN == 0.0){
		Specular = vec3(0.0);
	}
	else{
		Specular = Ms * clamp((1 / (4 * 3.1415 * alphaT * alphaB * sqrt(dotVN / dotLN))) * exp(-(dotHTAlphaX * dotHTAlphaX + dotHBAlphaY * dotHBAlphaY) / (dotHN * dotHN)), 0.0, 1.0);
	}
